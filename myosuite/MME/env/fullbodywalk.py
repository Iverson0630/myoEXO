...
class FullBodyWalkEnvV0(BaseV0):
    ...
    def get_obs_dict(self, sim):
        ...
        if hasattr(self, "qvel_ref"):
            qvel[6:] = sim.data.qvel[6:] - self.qvel_ref[7:]
    ...
    def _get_target_joint_rwd(self):
        joint = []
        ref_joint = []
        for bvh_name, mj_mappings in self.mapping.items():
            for mj_name, _, _ in mj_mappings:
                try:
                    jid = self.sim.model.joint_name2id(mj_name)
                except Exception:
                    continue
